...
```bash
docker build -t thuyadminrname/node-api:v1 ./node-api
docker build -t thuyadmin/python-api:v1 ./python-api
docker build -t thuyadmin/react-frontend:v1 ./frontend
```

### 3. Push lên Docker Hub
```bash
docker push thuyadmin/node-api:v1
docker push thuyadmin/python-api:v1
docker push thuyadmin/react-frontend:v1
```
